...
const getMessageSortTime = (msg) => {
  if (!msg) return 0;
  if (typeof msg.absoluteTimestamp === "number" && msg.absoluteTimestamp > 0) {
...
const analyzeTaskAndEstimateCost = (transcript) => {
  ...
  const hasUrgency = /urgent|asap|today|tomorrow|within (hours|day)|rush/.test(
    lowerText,
  );
  const hasRevisions =
...
/**
 * Remove all exclamation marks from response text to make it sound more human and less AI-generated.
 * Replace them with periods or other punctuation to maintain readability.
 */
const removeExclamationMarks = (text) => {
  if (!text || typeof text !== "string") return "";

  let result = text;

  // Replace exclamation marks followed by periods with just a period
  result = result.replace(/!+\./g, ".");

  // Replace exclamation marks at end of sentences/text with periods
  result = result.replace(/!+$/gm, ".");

  // Replace exclamation marks followed by space and capital letter (new sentence) with period
  result = result.replace(/!+(?=\s+[A-Z])/g, ".");

  // Replace remaining exclamation marks with periods
  result = result.replace(/!+/g, ".");

  // Clean up multiple consecutive periods (keep max 1 period or ellipsis)
  result = result.replace(/\.{4,}/g, "...");
  result = result.replace(/\.{3,}/g, "...");
  result = result.replace(/\.\.{2,}/g, ".");

  // Clean up spaces before periods
  ...
  cleaned = cleaned
    ...
    .filter((line) => {
      ...
      if (
        ...
        !/[a-zA-Z]{3,}/.test(trimmed.replace(/^\d+\.\s*[-–—]\s*/, ""))
      ) {
        return false;
      }
...
export const obfuscateSensitiveTerms = (text) => {
  ...
  result = result.replace(
    ...
    (email) =>
      isAlreadyObfuscated(email) ? email : obfuscateAlphanumericSegments(email),
  );

  result = result.replace(/(?:\+?\d[\d\s().-]{5,}\d|\b\d{7,}\b)/g, (match) => {
  ...
  if (phrasePattern) {
    result = result.replace(
      new RegExp(`\\b(${phrasePattern})\\b`, "gi"),
      (match) => {
        if (isAlreadyObfuscated(match)) return match;
        return match
          .split(/\s+/)
  ...
  if (wordPattern) {
    result = result.replace(
      ...
      (match) =>
        isAlreadyObfuscated(match)
          ? match
          : obfuscateWordWithDoubleHyphens(match),
    );
  }
...
const isMaskedKey = (value) => typeof value === "string" && value.includes("*");
...
const normalizeGeminiModel = (model) => {
  ...
  if (
    ...
    UNSUPPORTED_GEMINI_MODELS.has(trimmed)
  ) {
...
export const generateAiImage = async ({
  ...
  return {
    text: removeExclamationMarks(
      stripFencesAndPreamble(payload.text || "Here is the generated image."),
    ),
...
export const getAiChatResponse = async ({
  ...
  const wantsImage =
    ...
    (!presetKind &&
      ...
      (!hasAttachments || attachments.some((item) => item.kind === "image")));
  ...
  if (mode !== "meta" && cleaned) {
    ...
    cleaned = removeExclamationMarks(cleaned);
  }

  return {
    text: removeExclamationMarks(
      cleaned || (payload.images?.length ? "Here is the generated image." : ""),
    ),
